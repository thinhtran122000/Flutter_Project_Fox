...
*/
  ...
  A.redirectWithoutC():this.withoutC(10,10);
  A.redirectWithoutBC():this.withoutBC(20);
  A.redirectWithoutAll():this.withoutAll();
}
...
/* 
=> Khác lớp và kế thừa:
  + Giống với dạng các constructor ở cùng 1 lớp, redirecting constructor cũng không có body
  + Lớp được tạo có gán giá trị trước cho các thuộc tính trong field data của constructor bằng dấu ":". 
*/
class B {
  int a;
...
class C extends B {
  C():super.withoutC(10,10);
  C.withoutBC() : super.withoutBC(20);
  C.withoutAll():super.withoutAll();
}
/*
+ Khi truy cập đến 1 constructor có field data, redirecting constructor phải truyền giá trị phù hợp 
  với kiểu dữ liệu của field và cũng tuân thủ theo các kiểu field trong constructor (required và optional)
...
void main(){
  ...
  // -------------------------------
  var redirectWithoutC = A.redirectWithoutC();
  var redirectWithoutBC = A.redirectWithoutBC();
  var redirectWithoutAll = A.redirectWithoutAll();
  var c = C();
  var withoutBC = C.withoutBC();
  var withoutAll = C.withoutAll();
}
